...
const userService = {
  ...
  //user registration 
  registerUser: async (email, password, firstName, lastName,zipCode, utility) => {
    try {
        ...
        const result = await userRepo.registerUser(email, password, firstName, lastName, zipCode, utility);
        if (result) {
